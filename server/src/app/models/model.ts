import { Database, type QueryResultRow } from '#providers/database';
...
export abstract class Model<T extends QueryResultRow> {
    ...
    async all(): Promise<T[]> {
        ...
        const result = await Database.query<T>(queryStr);
    ...
    async find(filter: Filter<T>): Promise<T[]> {
        ...
        const result = await Database.query<T>(queryStr, values);
    ...
    async update(filter: Filter<T>, data: Partial<T>): Promise<number> {
        ...
        const queryStr = `
            ...
            SET ${setClause}
            ${sql}`;
        const result = await Database.query(queryStr, [...setValues, ...values]);
